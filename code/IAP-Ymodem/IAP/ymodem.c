...
int32_t Ymodem_Receive (uint8_t *buf)
{
	...
	session_done = 0, errors = 0, session_begin = 0;
	for (; ;)
	{
		packets_received = 0, file_done = 0, buf_ptr = buf;
		for (; ;)
		{
					...
					else
					{
						...
						if (packets_received == 0)
						{
							if (packet_data[PACKET_HEADER] != 0)
							{
								...
								STMFLASH_EraseSectorToAddr(APPLICATION_ADDRESS, size);                    
								Send_Byte(ACK);
								Send_Byte(CRC16);
							}
						...
						else
						{
							...
							if(STMFLASH_Write(&flashdestination,(uint32_t*) ramsource, packet_length/4) ==0)
							{
								Send_Byte(ACK);
							}
