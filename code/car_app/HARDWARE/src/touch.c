...
void TP_Save_Adjdata(void)
{
	...
	tp_dev.check=0x0a;
	memcpy((u8*)&buf,&tp_dev.xfac,14);
	
	RTC_Write_BKR(11,buf[0]);
	RTC_Write_BKR(12,buf[1]);
	RTC_Write_BKR(13,buf[2]);
	RTC_Write_BKR(14,buf[3]);
	RTC_Write_BKR(15,buf[4]);
...
{	
	...
	buf[0] = RTC_Read_BKR(11);
	buf[1] = RTC_Read_BKR(12);
	buf[2] = RTC_Read_BKR(13);
	buf[3] = RTC_Read_BKR(14);
	buf[4] = RTC_Read_BKR(15);
	
	memcpy(&tp_dev.xfac,(u8*)&buf,14);
...
{	
	...
	GPIO_Set(GPIOE,PIN0|PIN1,GPIO_MODE_IN,0,0,GPIO_PUPD_PU);						//PB1/PB2 设置为上拉输入
	GPIO_Set(GPIOE,PIN2|PIN3|PIN4,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU); 	//PB0设置为推挽输出
