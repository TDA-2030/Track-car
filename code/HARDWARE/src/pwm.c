...
#define TURN_MAX_ANGLE 20
#define TURN_MIN_ANGLE -20
...
void TIM4_PWM_Init()
{
	 ...
	 TIM_OCInitStructure.TIM_Pulse = 200;  //占空比为0
...
void TIM5_PWM_Init()//PA输出PWM
{
	...
	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1; //选择定时器模式:TIM脉冲宽度调制模式1
 	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable; //比较输出使能
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High; //输出极性:TIM输出比较极性高
	TIM_OCInitStructure.TIM_Pulse = 1500;  //初始化占空比为0
	...
	TIM_CtrlPWMOutputs(TIM5,ENABLE);//设置PWM输出为使能
	
	TIM_Cmd(TIM5, ENABLE);  //使能TIM5
}

//人体动作控制舵机角度设置函数
void Servor_Act_Angle(u8 num,int8_t angle)//num:通道编号
{
	uint16_t CCR;//比较值与高电平时间
...
void TIM2_PWM_Init()
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10|GPIO_Pin_11; //TIM2的通道1、2
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;        
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;	
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;      
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;  
	...
	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable; //比较输出使能
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High; //输出极性:TIM输出比较极性高
	TIM_OCInitStructure.TIM_Pulse = 1500;  //占空比为0
	...
	TIM_CtrlPWMOutputs(TIM2,ENABLE);//设置PWM输出为使能
...
void Servor_Turn_Angle(u8 fx,s8 angle)//fx:舵机方向
{
	...
	switch(fx)
	{
		case TURN_SERVO_FORWARD :TIM_SetCompare3(TIM2,(u32)CCR);break;
		case TURN_SERVO_BACKWARD:TIM_SetCompare4(TIM2,(u32)CCR);break;
	}
}
